...
# 
#     Siga as instruções na tela para configurar as preferências de backup, como a escolha entre backups automáticos ou manuais e a definição do tipo de backup (incremental, baseado em `RSYNC` ou `BTRFS`).
# 
...
# O `Timeshift` é inteligente o suficiente para não duplicar arquivos desnecessariamente. Ele utiliza um método chamado "hard links" para as cópias de backup quando está operando em sistemas de arquivos que suportam esse recurso, como é o caso do ext4.
# 
# Aqui está o que acontece quando você marca mais de uma opção de backup, como diário, semanal e mensal:
# 
# - **Incrementais**: O `Timeshift` cria backups incrementais. Isto significa que após o primeiro backup completo, os backups subsequentes apenas salvam as diferenças em relação ao backup anterior.
# 
# - **Hard Links**: Para arquivos que não mudaram entre snapshots, o `Timeshift` usa hard links. Com hard links, vários arquivos em diferentes snapshots podem apontar para o mesmo espaço no disco, economizando espaço. Só é feita uma nova cópia se o arquivo original for alterado.
# 
...
# Portanto, mesmo marcando diferentes níveis de snapshots para serem mantidos, o `Timeshift` irá gerenciar inteligentemente o espaço e não criará cópias duplicadas dos arquivos, a menos que haja mudanças neles. Isso otimiza o uso do espaço em disco e mantém os backups organizados e eficientes.

# ### 1.1 `Type`
# 
# Nesta tela do `Timeshift` você pode escolher entre dois tipos de tecnologias de snapshots:
# 
# - **RSYNC**: Este é um método de backup incremental que copia as diferenças entre o estado atual do sistema e o último snapshot. Ele suporta qualquer tipo de sistema de arquivos e é mais flexível em termos de armazenamento, uma vez que os backups podem ser salvos em qualquer local, incluindo unidades externas ou em outra partição. No entanto, como ele copia arquivos, pode ser um pouco mais lento e consumir mais espaço do que o `BTRFS` para as mesmas operações de backup.
# 
# - **BTRFS**: É um sistema de arquivos com funcionalidades avançadas, incluindo a capacidade de criar snapshots quase instantâneos que não duplicam dados. Seu método de "copy-on-write" permite que os backups sejam extremamente rápidos e economizem espaço, uma vez que apenas as alterações desde o último snapshot são armazenadas. Contudo, para usar `BTRFS`, a partição raiz do seu sistema precisa estar usando o sistema de arquivos `BTRFS`, e os snapshots são salvos na mesma partição.
# 
# A escolha entre `RSYNC` e `BTRFS` depende do seu sistema de arquivos e de como você deseja gerenciar seus backups. Se você precisa de flexibilidade e está usando diferentes tipos de sistemas de arquivos, ou se deseja armazenar seus backups em locais diferentes, o RSYNC é a melhor escolha. Por outro lado, se você já está usando `BTRFS` e deseja a forma mais rápida e eficiente de armazenar snapshots no mesmo dispositivo, a opção `BTRFS` seria ideal.

# ### 1.2 `Location`
# 
# Esta tela do Timeshift é onde você seleciona o local onde os snapshots (ou seja, os backups do estado do sistema) serão armazenados. Aqui estão os elementos principais que você pode observar na interface:
# 
...
# 
# - **Informação Importante**: Na parte inferior da janela, há avisos e restrições importantes sobre a seleção do local do snapshot. Por exemplo, os snapshots são salvos na pasta `/timeshift` na partição selecionada, e locais remotos ou de rede não são suportados.
# 
...
# ### 1.3 `Schedule`
# 
# Nesta tela do Timeshift, você pode definir a frequência e a quantidade de snapshots que deseja manter no sistema. Vamos detalhar cada parte:
# 
# - **`Select Snapshot Levels` (Selecionar Níveis de Snapshot)**: Aqui você pode marcar quais tipos de snapshots quer que o Timeshift crie e mantenha automaticamente: mensal, semanal, diário, horário e a cada inicialização do sistema (boot).
# 
...
# 
# - **`Stop cron emails for scheduled tasks` (Parar e-mails do cron para tarefas agendadas)**: Se marcado, o sistema não enviará e-mails automáticos sempre que uma tarefa cron relacionada ao Timeshift for executada. Isso pode ser útil para evitar spam no seu e-mail se você tiver o sistema configurado para enviar e-mails de cron jobs.
# 
# - **Informações Adicionais**: Na parte inferior, há informações importantes sobre como os snapshots são agendados e criados:
# 
#     - Os snapshots não são agendados em horários fixos, mas sim conforme a necessidade com base na frequência configurada.
# 
#     - Uma tarefa de manutenção é executada a cada hora para criar snapshots, se necessário.
# 
#     - Snapshots de boot são criados com um atraso de 10 minutos após a inicialização do sistema.
# 
# - **`Scheduled snapshots are enabled` (Snapshots agendados estão ativados)**: Esta mensagem indica que os snapshots automáticos estão configurados e serão criados se houver espaço suficiente no disco (mais de 1 GB).
# 
# Lembre-se de que a manutenção de múltiplos snapshots pode ocupar uma quantidade significativa de espaço no disco, então assegure-se de ter espaço suficiente na partição selecionada para armazená-los.

# ### 1.4 `User`
# 
# Na tela de configurações do `Timeshift` que você está mostrando, há opções para gerenciar como os diretórios home dos usuários são incluídos nos snapshots:
# 
# - **`Exclude All Files` (Excluir Todos os Arquivos)**: Se selecionada, esta opção exclui todos os arquivos no diretório home do usuário do snapshot. Isso é útil se você deseja apenas fazer backup do sistema sem os dados pessoais dos usuários.
# 
...
#     
# Essa funcionalidade é útil para personalizar seus backups, permitindo que você exclua arquivos temporários ou desnecessários e inclua documentos importantes e configurações do sistema. Isso ajuda a economizar espaço em disco e a criar snapshots mais rápidos e eficientes.

# ### 1.6 `Misc`
# 
# Esta tela nas configurações do Timeshift permite que você escolha o formato de data e hora usado para nomear os snapshots.
# 
